...
def gptq_fwrd(model, dataloader, dev, args):
    ...
    expand = AbsorbingDict(config_mix['attention'])

    dtype = next(iter(model.parameters())).dtype
    # In our case we expand dimension so input and output can be differents sizes.
    # To store every input possible we create a tensor with the maximum dimension it can reach and slice the corresponding dimension when we apply layer
    max_expand = max(list(expand.values()))
    inps = torch.zeros(
        (args.nsamples, model.seqlen, model.config.hidden_size + max_expand), dtype=dtype, device=dev
    )
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            inps[cache['i'], :, :model.config.hidden_size + expand['0']] = inp
            cache['i'] += 1
    ...
    for i in range(n_layers):
        ...
        l_out = i+1 if i+1 <= n_layers-1 else n_layers - 1
        out_dim = model.config.hidden_size + expand[str(l_out)]
        in_dim = model.config.hidden_size + expand[str(i)]
        for names in sequential:
            ...
            for j in range(args.nsamples):
                outs[j, :, :out_dim] = layer(inps[j, :, :in_dim].unsqueeze(0), attention_mask=attention_mask, position_ids=position_ids)[0]
            for h in handles:
                h.remove()
