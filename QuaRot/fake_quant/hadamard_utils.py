...
class Hadamard_block(nn.Module):
    ...
    def set(self, H, expand, hidden_dim):
        self.H = H
        self.expand = expand
    ...
    def forward(self, x):
        ...
        if self.expand is not None:
            x_padded = F.pad(x, (0, self.expand, 0, 0))
            return x_padded @ self.H
        elif self.hidden_dim is not None:
            x_H = x @ self.H
            return x_H[:,:,:self.hidden_dim]
def hadamard_transform(x, scale):
    dim = x.shape[-1]
    return torch.nn.functional.linear(x, torch.tensor(scipy.linalg.hadamard(dim)).to(device=x.device, dtype=x.dtype)) * scale
...
def get_hadK(n, transpose=False):
    ...
    if is_pow2(n):
        K = 1
    elif (n - 1) % 4 == 3 and isprime(n - 1):  # expand with paley algorithm : 4567, 4679
        K = n
        if os.path.exists('./save/hadK/hadK_' + str(n) + '.pth'):
            hadK = torch.load('./save/hadK/hadK_' + str(n) + '.pth').cuda()
        else:
            hadK = hadamard_paley(n - 1).T if transpose else hadamard_paley(n - 1)
            torch.save(hadK, './save/hadK/hadK_' + str(n) + '.pth')
        ...
        if output:
            ...
            if module.bias is not None:
                ...
                b_ = fast_hadamard_transform.hadamard_transform(
                    b_.reshape(b_dim//had_dim, had_dim), 
                    scale=1/math.sqrt(had_dim)
                    ).reshape(b_dim)
        else:
