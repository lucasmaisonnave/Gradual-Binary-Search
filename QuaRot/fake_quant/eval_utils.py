...
@torch.no_grad()
def evaluator(model, testenc, dev, args, past_key_values=None):#
    ...
    if 'opt' in args.model:
        opt_type = True
        llama_type = False
    elif 'meta' in args.model or 'deepseek' in args.model or 'mistral' in args.model or 'Qwen' in args.model:
        llama_type = True
        opt_type = False
    else:
    ...
    for i in tqdm(range(len(layers)), desc="(Eval) Layers"):
        ...
        for j in range(nbatches):
            if opt_type:
                ...
                outs[j] = layer(inps[j], attention_mask=attention_mask[0].unsqueeze(0), position_ids=position_ids)[0] # , prefixed_key_values=prfx_kv
        # layers[i] = layer.cpu()
        del layer
        torch.cuda.empty_cache()
        inps, outs = outs, inps
