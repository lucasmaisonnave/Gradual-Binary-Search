...
def model_type_extractor(model):
    if isinstance(model, LLAMA_MODEL):
        ...
        return OPT_MODEL
    elif isinstance(model, PHI_MODEL):
        return PHI_MODEL
    else:
...
def get_rope_function_name(model):
    if isinstance(model, LLAMA_MODEL) or isinstance(model, QWEN_MODEL) or isinstance(model, MISTRAL_MODEL):
...
def get_layers(model):
    ...
    if isinstance(model, MISTRAL_MODEL):
        return model.model.layers
    if isinstance(model, PHI_MODEL):
        return model.model.layers
    raise NotImplementedError


def get_llama(model_name, hf_token, cache_dir):
...
def get_mistral(model_name, hf_token, cache_dir):
    torch.nn.init.kaiming_uniform_ = skip
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
...
def get_qwen(model_name, hf_token, cache_dir):
    torch.nn.init.kaiming_uniform_ = skip
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype='auto',
...
def get_phi(model_name, hf_token, cache_dir):
    torch.nn.init.kaiming_uniform_ = skip
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype='auto',
...
def get_opt(model_name, cache_dir = None):
    torch.nn.init.kaiming_uniform_ = skip
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
...
def get_model(
    model_name, hf_token=None, cache_dir=None
):
    if 'llama' in model_name:
        return get_llama(model_name, hf_token, cache_dir)
    elif 'mistral' in model_name:
        return get_mistral(model_name, hf_token, cache_dir)
    elif 'Qwen' in model_name:
        return get_qwen(model_name, hf_token, cache_dir)
    elif 'Phi' in model_name:
        return get_phi(model_name, hf_token, cache_dir)
...
def get_model_type(model):
    if isinstance(model, OPT_MODEL):
        ...
        model_type = PHI_MODEL
    else:
        raise ValueError(f'Unknown model type {model}')
    return model_type

def get_embeddings(model, model_type) -> list[torch.nn.Module]:
    if model_type == LLAMA_MODEL or model_type == QWEN_MODEL or model_type == PHI_MODEL or model_type == MISTRAL_MODEL:
...
def get_pre_head_layernorm(model, model_type):
    if model_type == LLAMA_MODEL or model_type == QWEN_MODEL or model_type == PHI_MODEL or model_type == MISTRAL_MODEL:
        ...
        assert isinstance(pre_head_layernorm,
                          transformers.models.llama.modeling_llama.LlamaRMSNorm) or isinstance(pre_head_layernorm,
                          transformers.models.qwen2.modeling_qwen2.Qwen2RMSNorm) or isinstance(pre_head_layernorm,
                          transformers.models.mistral.modeling_mistral.MistralRMSNorm) ## or isinstance(pre_head_layernorm, transformers.models.phi3.modeling_phi3.Phi3RMSNorm)
    elif model_type == OPT_MODEL:
...
def capture_layer_io(model_type, layer, layer_input):
    ...
    if model_type == LLAMA_MODEL or model_type == QWEN_MODEL or model_type == MISTRAL_MODEL:
        captured_inputs = {
