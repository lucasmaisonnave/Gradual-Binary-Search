...
class ActQuantWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.module(x.cuda()).to(x_dtype)
...
def remove_actquant(root_module):
    """Remove ActQuantWrapper layers and restore original layers iteratively"""
    ...
    while module_stack:
        ...
        if hasattr(module, '_original_layers'):
            for attr, original_layer in module._original_layers.items():
                setattr(module, attr, original_layer)
            delattr(module, '_original_layers')
        
        if hasattr(module, '_processed_actquant'):
            delattr(module, '_processed_actquant')
        
        # Add direct children to stack
        for _, child in module.named_children():
            module_stack.append(child)


def cleanup_model(model):
    """Full cleanup of the model"""
    remove_actquant(model)  # First remove all wrappers
