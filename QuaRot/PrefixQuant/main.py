...
def main():
    ...
    parser.add_argument("--cache_dir_hub", type=str,default="/data1/is156025/lm270675/.cache/huggingface/hub", help="exampe:piqa,arc_easy,arc_challenge,hellaswag,winogrande")
    ...
    if args.resume_quant:
        ...
        model, tokenizer = load_quantized_model(args.resume_quant,args.wbits, args.group_size, args.cache_dir)
    else:
        # load fp quantized model
        config = AutoConfig.from_pretrained(args.model_path,trust_remote_code=True,cache_dir=args.cache_dir_hub)
        ...
        model = AutoModelForCausalLM.from_pretrained(args.model_path, config=config, device_map='cpu',torch_dtype=dtype,trust_remote_code=True,cache_dir=args.cache_dir_hub)
        if args.pre_rotate:
    ...
    if args.save_quant_dir:
        logger.info("start saving prefix")
