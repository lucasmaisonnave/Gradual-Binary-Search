...
def get_pile(tokenizer, train_size, val_size, seed, seqlen, cache_dir=None):
    print("get_pile")
    traindata = load_dataset("mit-han-lab/pile-val-backup", split="validation", cache_dir=cache_dir)
...
def get_wikitext2(tokenizer, train_size, val_size, seed, seqlen, test_only, cache_dir=None):
    print("get_wikitext2")
...
def get_c4(tokenizer, train_size, val_size, seed, seqlen, test_only, cache_dir=None):
    print("get_c4")
    try:
...
def get_redpajama(tokenizer, train_size, val_size, seed, seqlen, cache_dir=None):
    print("get_redpajama")
    try:
        loacal_dataset = "/cpfs01/user/chenmengzhao/huggingface/datasets/togethercomputer___red_pajama-data-1_t-sample"
        traindata = load_dataset(loacal_dataset,split='train', cache_dir=cache_dir)   
    except:
        traindata = load_dataset("togethercomputer/RedPajama-Data-1T-Sample",split='train', cache_dir=cache_dir)   
    random.seed(seed)
    traindata = traindata.shuffle(seed=seed) 
    trainloader = []
    val_sample_ratio = 0.9
...
def get_loaders(
    name, tokenizer, train_size=128, val_size=64,seed=0, seqlen=2048, test_only=False, cache_dir=None
):
    if 'wikitext2' in name:
        return get_wikitext2(tokenizer,train_size,val_size,seed,seqlen,test_only,cache_dir)
    elif 'c4' in name:
        return get_c4(tokenizer,train_size,val_size,seed,seqlen,test_only, cache_dir)
    elif 'redpajama' in name:
        return get_redpajama(tokenizer,train_size,val_size,seed,seqlen, cache_dir)
    elif 'pile' in name:
        return get_pile(tokenizer,train_size,val_size,seed,seqlen, cache_dir)
    else:
...
@torch.no_grad()
def test_ppl(args, model, tokenizer,prefixed_key_values=None, datasets=['wikitext2'], cache_dir=None):
    results = {}
