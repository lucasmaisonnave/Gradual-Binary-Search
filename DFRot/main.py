...
from GradualBinarySearch import GBS, update_quantizer
...
def main():
    ...
    if args.gbs:
        model.cuda()
        gbs = GBS(args, model, tokenizer, b=[args.a_bits])
        if not os.path.exists(gbs.folder + 'best_config_' + gbs.filename):
            config_gbs = gbs.search()['config']
        else:
            import json
            with open(gbs.folder + 'best_config_' + gbs.filename, 'r') as file:
                config_gbs = json.load(file)[0]['config']
        update_quantizer(args, model, config_gbs)
    ...
    nouvelle_ligne = [args.model, args.w_bits, args.gbs, dataset_ppl]
    ...
    L = [args.model,args.w_bits,args.gbs]
