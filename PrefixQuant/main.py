...
def main():
    ...
    if args.resume_quant:
        ...
        config = AutoConfig.from_pretrained(args.model_path,trust_remote_code=True, cache_dir=cache_dir)
        tokenizer = AutoTokenizer.from_pretrained(args.model_path, use_fast=False,legacy=False,trust_remote_code=True, cache_dir=cache_dir)
        dtype = torch.float16 if not args.use_fp32 else torch.float32
        model = AutoModelForCausalLM.from_pretrained(args.model_path, config=config, device_map='cpu',torch_dtype=dtype,trust_remote_code=True, cache_dir=cache_dir)
        if args.pre_rotate:
