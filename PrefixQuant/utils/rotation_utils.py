...
def fuse_layer_norms(model, rmsn = True):
...
def rotate_embeddings(model, Q: torch.Tensor) -> None:
    ...
    for W in model_utils.get_embeddings(model, model_type):
        ...
        W.weight.data = torch.matmul(W_, Q).to(device=DEV, dtype=dtype) 
...
def rotate_faster_down_proj(layer, model_type, hardK):
    ...
    W.weight.data = W.weight.data.to(device=DEV, dtype=dtype)
...
def rotate_ov_proj(layer, Q, model_type, num_attention_heads, num_key_value_heads, head_dim):
    ...
    v_proj.weight.data = torch.matmul(Q.T.unsqueeze(0), W_).reshape(shape).to(device=DEV, dtype=dtype)
    if v_proj.bias is not None:
        ...
        v_proj.bias.data = torch.matmul(Q.T, B_).flatten().to(device=DEV, dtype=dtype)
    ...
    o_proj.weight.data = torch.matmul(W_, Q).reshape(shape).to(device=DEV, dtype=dtype)
